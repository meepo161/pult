...
//Испытание изоляции соединительных проводов
class Test3(model: MainViewModel, view: TestView, controller: TestController) : Test(model, view, controller) {
        ...
        if (isTestRunning) {
            val timer =
                CallbackTimer(tickPeriod = 1.seconds, tickTimes = model.testObject.value.objectTime.toInt(),
                    ...
                    tickJob = {
                        if (isTestRunning) {
                            if (model.selectedConnectionPoints.isNotEmpty()) {
    ...
    private fun markChannels() {
        model.selectedConnectionPoints[MainViewModel.CONNECTION_1]?.let {
        ...
        model.selectedConnectionPoints[MainViewModel.CONNECTION_2]?.let {
        ...
        model.selectedConnectionPoints[MainViewModel.CONNECTION_3]?.let {
        ...
        model.selectedConnectionPoints[MainViewModel.CONNECTION_4]?.let {
